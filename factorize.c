...
static factoralts( struct dictnode *z);
static struct hypernotion *copyhn(struct hypernotion *,int,int);
static int substringlen( struct hypernotion *hn1, struct hypernotion *hn2);
static bool samessm( struct hitem *x1, struct hitem *x2);
static void addalt( struct dictnode *z, struct hypernotion *hn);
static void deletealt( struct dictnode *z, struct hypernotion *hn);


global void factorize(void )
  { bool again;
    anon = 0;
    do
      { struct dictnode *z;
...
static struct hypernotion *copyhn( struct hypernotion *old, int k1, int k2)
