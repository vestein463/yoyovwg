...
global struct hyperrule *substrule( struct hyperrule *old);

static bool parsehn( struct hypernotion *hnot, int hptr, int hend, int level);
static bool parsemeta( struct hypernotion *hnot, int hptr, int level);
static prcontext( struct hypernotion *hn, int ptr, int end);
static struct hyperalt *substalt(struct hyperalt *old);
static struct hypernotion *substhn(struct hypernotion *old);

global bool hnmatch( struct hypernotion *pn, struct hypernotion *hn)
  { bool ok;
    if (debugbits & 0x10) fprintf(stdout,"\nhnmatch pn=%h   hn=%h\n", pn, hn);
...
global bool eqhn( struct hypernotion *hn1, struct hypernotion *hn2)
